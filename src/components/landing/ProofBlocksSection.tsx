...
export function ProofBlocksSection({ navigate }: { navigate: (href: string) => void }) {
  ...
  const content = (
    <>
      ...
      <blockquote className="mt-8 rounded-[var(--radius-md)] border-l-2 border-l-[var(--color-accent)] bg-[var(--color-bg-elevated)] p-6 text-left">
      ...
      <div className="mt-8 grid gap-6 sm:grid-cols-3">
        {PROOF_POINTS.map((point) => (
          <div
            key={point.stat}
            className="rounded-[var(--radius-md)] bg-[var(--color-bg-surface)] p-6 text-center"
          >
