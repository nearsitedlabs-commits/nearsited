...
export function Toast({ message, onClose, duration = 3000, type = "success" }: ToastProps) {
  ...
  if (prefersReduced) {
    return (
      <div className="fixed bottom-20 right-4 sm:bottom-6 sm:right-6 z-50 flex items-center gap-3 rounded-[var(--radius-md)] bg-[var(--color-bg-surface)] px-5 py-3.5 text-sm font-medium text-[var(--color-text-primary)] shadow-[var(--brand-shadow-lg)]">
      ...
      <AnimatePresence>
        {visible && (
          <motion.div
            ...
            className="flex items-center gap-3 rounded-[var(--radius-md)] bg-[var(--color-bg-surface)] px-5 py-3.5 text-sm font-medium text-[var(--color-text-primary)] shadow-[var(--brand-shadow-lg)]"
          >
            <span className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full ${typeStyle.bg}`}>
              {typeStyle.icon}
            </span>
