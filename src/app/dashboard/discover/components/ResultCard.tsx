...
const _TIER_COLORS: Record<string, { ring: string; text: string }> = {
  high: { ring: "border-[var(--color-success)]", text: "text-[var(--color-success)]" },
  medium: { ring: "border-[var(--color-info)]", text: "text-[var(--color-info)]" },
  ...
  return (
    ...
    >
      ...
      {business.rating != null && !isAnalyseLoading && (
        <div className="flex-shrink-0 text-[11px] text-[var(--color-text-tertiary)] whitespace-nowrap">
          <span className="text-[var(--color-warning)]">★</span>{" "}
          {business.rating.toFixed(1)}
          {business.review_count != null && business.review_count > 0 && (
            <span className="text-[var(--color-text-tertiary)] ml-0.5">
